...
MASTER_STOCKS = [
    ...
    {"ticker": "FLTRL.XC", "name": "Flutter Entertainment plc", "exchange": "NYS", "currency": "GBp"},  # New York listing
...
st.write("View stock prices with 5-day % change and YTD % change.")
...
if st.button("Run"):
    ...
    for stock in SELECTED_STOCKS:
        ticker = stock["ticker"]
        try:
            # Download adjusted close prices from last trading day of previous year
            start_year = f"{date_str.year-1}-12-31"
            end_date = date_str + timedelta(days=1)
            data = yf.download(ticker, start=start_year, end=end_date, progress=False)
            if data.empty:
                continue
            ...
            price = float(data.loc[sel_date, "Adj Close"])

            # 5-day % change
            past_dates = data.index[data.index <= sel_date - timedelta(days=5)]
            change_5d = None
            if len(past_dates) > 0:
                past_price = float(data.loc[past_dates[-1], "Adj Close"])
                change_5d = (price - past_price) / past_price * 100

            # YTD % change using last trading day of previous year
            ytd_price = float(data.iloc[0]["Adj Close"])
    ...
    if rows:
        ...
        # --- CSV download formatted like on-screen display ---
        output_lines = []
        for (exchange, currency), gdf in grouped:
